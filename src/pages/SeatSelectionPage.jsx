    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center">
          ...
          <h2 className="font-display text-2xl font-bold text-gray-900 mb-1">Booking Confirmed!</h2>
          <p className="text-gray-500 mb-6">Your bus ticket has been booked successfully</p>
          <div className="bg-gray-50 rounded-2xl p-4 text-left space-y-2 mb-6 text-sm">
            ...
            <div className="flex justify-between"><span className="text-gray-500">Seats</span><span className="font-semibold">{success.booking?.seat_numbers?.join(', ')}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Amount Paid</span><span className="font-bold text-green-600">₹{parseFloat(success.booking?.total_amount).toLocaleString()}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Transaction ID</span><span className="font-mono text-xs">{success.transaction_id}</span></div>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-6 py-4">
          ...
          {bus && (
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-display font-bold text-gray-900">{bus.bus_name} — {bus.routes?.source} → {bus.routes?.destination}</h1>
                <p className="text-sm text-gray-500">{dep && format(dep,'HH:mm')} → {arr && format(arr,'HH:mm')} · {bus.bus_type}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2">
          {step === 1 && (
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h2 className="font-semibold text-gray-900 mb-4">Select Your Seats</h2>
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="text-center text-xs text-gray-400 mb-4 pb-3 border-b border-gray-200">
                  🚌 Driver's Side (Front)
                </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 sticky top-24">
            ...
            {bus && (
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Route</span><span className="font-medium text-right">{bus.routes?.source} → {bus.routes?.destination}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Bus</span><span className="font-medium">{bus.bus_name}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Type</span><Badge color="ocean">{bus.bus_type}</Badge></div>
                <div className="flex justify-between"><span className="text-gray-500">Date</span><span className="font-medium">{params.get('date')}</span></div>
                {dep && <div className="flex justify-between"><span className="text-gray-500">Departure</span><span className="font-medium">{format(dep,'HH:mm')}</span></div>}
                <hr className="border-gray-100" />
                <div className="flex justify-between"><span className="text-gray-500">Seats</span>
                  <span className="font-medium">{selected.length > 0 ? selected.map(s => s.seat_number).join(', ') : 'None selected'}</span>
                </div>
                <div className="flex justify-between"><span className="text-gray-500">Price/seat</span><span>₹{bus.price_per_seat}</span></div>
                <hr className="border-gray-100" />
