...
function BusCard({ bus, onSelect }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all p-5 flex flex-col md:flex-row items-start md:items-center gap-4">
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div>
            <h3 className="font-semibold text-gray-900">{bus.bus_name}</h3>
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            <div className="text-xl font-bold text-gray-900">{format(dep, 'HH:mm')}</div>
          ...
          <div className="text-center">
            <div className="text-xl font-bold text-gray-900">{format(arr, 'HH:mm')}</div>
        ...
        {bus.amenities?.length > 0 && (
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            {bus.amenities.slice(0,4).map(a => (
              <span key={a} className="flex items-center gap-1 text-xs bg-gray-50 text-gray-500 px-2 py-0.5 rounded-full">
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="font-display text-xl font-bold text-gray-900">{source} → {destination}</h1>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center gap-2 mb-5">
          ...
          {['', 'AC', 'Non-AC', 'Sleeper', 'Semi-Sleeper', 'Volvo'].map(t => (
            <button key={t} onClick={() => setFilter(t)} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${filter === t ? 'bg-purple-600 text-white shadow' : 'bg-white text-gray-600 border border-gray-200 hover:border-purple-400'}`}>
              {t || 'All'}
            </button>
          ))}
        </div>
