...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label:'Total Users',       value: stats?.totalUsers ?? 0,          icon:Users,     bg:'bg-purple-50',  text:'text-purple-600',  border:'border-purple-100' },
    { label:'Bus Bookings',      value: stats?.totalBusBookings ?? 0,    icon:Bus,       bg:'bg-violet-50',  text:'text-violet-600',  border:'border-violet-100' },
    { label:'Tour Bookings',     value: stats?.totalPackageBookings ?? 0,icon:MapPin,    bg:'bg-purple-50',  text:'text-purple-600',  border:'border-purple-100' },
    ...
    <AdminLayout title="Dashboard">
      <div className="space-y-6 max-w-6xl">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Here's what's happening with TravelGo today.</p>
        </div>

        {/* Stat cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {statCards.map(({ label, value, icon:Icon, bg, text, border }) => (
            <div key={label} className={`bg-white rounded-2xl border ${border} p-5 shadow-card hover:shadow-purple-md transition-shadow`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-white rounded-2xl border border-purple-100 p-5 shadow-card">
            <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2"><Activity size={16} className="text-purple-500" /> Quick Actions</h3>
            <div className="space-y-2">
              {[
                ...
                <button key={to} onClick={() => navigate(to)} className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-purple-50 hover:bg-purple-100 transition-colors group text-left">
          ...
          <div className="bg-white rounded-2xl border border-purple-100 p-5 shadow-card">
            <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2"><Activity size={16} className="text-purple-500" /> System Status</h3>
