...
export default function BusSearchPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%)' }}>
      <div className="max-w-3xl mx-auto px-6 py-16">
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-4xl font-bold text-gray-900 mb-2">Search Bus Tickets</h1>
          <p className="text-gray-500">Find and book bus tickets at the best prices</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-purple-100 p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="flex items-end gap-3">
              ...
              <button type="button" onClick={swap} className="mb-0.5 p-3 rounded-xl border border-gray-200 hover:bg-purple-50 hover:border-purple-300 transition-colors text-gray-500 hover:text-purple-600">
          ...
          <div className="mt-8 pt-6 border-t border-gray-100">
            <p className="text-sm font-semibold text-gray-600 mb-3">Popular Routes</p>
            <div className="flex flex-wrap gap-2">
              {[['Chennai','Bangalore'],['Delhi','Jaipur'],['Mumbai','Pune'],['Bangalore','Goa']].map(([s,d]) => (
                <button key={`${s}-${d}`} onClick={() => setForm({source:s, destination:d, date:today})}
                  className="text-xs px-4 py-2 rounded-full font-semibold border-2 border-purple-200 text-purple-700 bg-purple-50 hover:bg-purple-600 hover:text-white hover:border-purple-600 transition-all">
                  {s} → {d}
                </button>
              ))}
            </div>
